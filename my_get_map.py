...
from multimode import Multimode_collaboration
...
if __name__ == "__main__":
    ...
    #   光谱库文件夹
    spectral_lib_path = 'spectral_lib'
    ...
    if not os.path.exists(os.path.join(map_out_path, 'multimode-detection-results')):
        os.makedirs(os.path.join(map_out_path, 'multimode-detection-results'))
    if not os.path.exists(os.path.join(map_out_path, 'temp-results')):
        os.makedirs(os.path.join(map_out_path, 'temp-results'))

    class_names, _ = get_classes(classes_path)

    if map_mode == 0 or map_mode == 1:
        # Yolo检测要获得尽可能多的预测框，囊括所有可能的目标，以得到较大的Recall
        print("Load model.")
        ...
        for image_id in tqdm(image_ids):
            ...
            #  加载配准的高光谱图像
            hyimg_dict = scio.loadmat("hyperspectral/" + image_id + ".mat")
            hyimg = hyimg_dict['data']
            #  高光谱图像和可见光图像进行多模协同检测
            Multimode_collaboration(map_out_path, image_id, hyimg)
    ...
    if map_mode == 0 or map_mode == 3:
        ...
        print("Get map done.")
        print("Get multimodal map.")
        get_map(MINOVERLAP, True, score_threhold=score_threhold, path=map_out_path,
                detection_path='multimode-detection-results')
        print("Get map done.")
